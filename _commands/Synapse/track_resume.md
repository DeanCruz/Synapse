...
### Step 4: Read the canonical plan

`{tracker_root}/dashboards/{dashboardId}/plan.json` contains the deeply-thought plan: shared `context` and the `tasks[]` array with each task's `id`, `title`, `description`, `approach`, `files`, and `depends_on`.

1. Read `{tracker_root}/dashboards/{dashboardId}/plan.json`.
2. If the file is missing or invalid (legacy swarm), regenerate it now: redo the deep-thinking pass over the original prompt (from `initialization.json`'s `task.prompt`) and write a fresh `plan.json` BEFORE re-dispatching any worker. Hooks block dispatch otherwise.
...
**For resume purposes, treat ALL `in_progress` tasks as stale** — if the master is running `!track_resume`, the previous master session is dead, which means all its dispatched workers are dead too. Re-dispatch everything that isn't `completed`.
...
1. READ YOUR TASK IN THE CANONICAL PLAN:
   ...
   Also read entries for tasks listed in your `depends_on` (their summaries are in upstream progress files).
...
## Quick Reference: `!track_resume` vs `!dispatch --ready` vs `!retry`

| Command | Scope | What it does |
|---|---|---|
| `!track_resume` | **Entire swarm** | Full assessment + re-dispatch ALL incomplete tasks (failed, stale, pending) with complete context. Becomes the new master orchestrator for the swarm. |
...
`!track_resume` is the "pick up where we left off" command. It combines the assessment depth of `!status`, the retry logic of `!retry`, and the dispatch mechanics of `!dispatch --ready` into a single operation that fully restores a stalled swarm.
