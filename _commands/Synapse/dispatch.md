...
5. **Read the canonical plan** at `{tracker_root}/dashboards/{dashboardId}/plan.json`. Extract `context` (shared) and the `tasks[]` entry whose `id == "{task_id}"` — that entry contains the deeply-thought `approach` and `files`.
