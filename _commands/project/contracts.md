...
Audits all API contracts between frontend and backend layers within the project. Lists every endpoint the frontend calls, verifies each has a backend handler, checks that request/response shapes match, and flags orphaned or undocumented endpoints.
...
Search the frontend layer of the project for all outbound API calls:
...
Search the backend layer of the project for all route definitions:

- Express/Hono/Fastify route handlers
- Firebase Cloud Functions HTTP triggers (`onRequest`, `onCall`)
- API route files (e.g., Next.js API routes, serverless functions)
...
|---|---|---|---|---|
| POST | /api/auth/login | `src/api/auth.ts:15` | `src/routes/auth/login.ts:8` | Match |
| GET | /api/users/:id | `src/api/users.ts:22` | `src/routes/users/get.ts:12` | Response drift |
| POST | /api/events | `src/api/events.ts:30` | — | No backend handler |
| DELETE | /api/admin/purge | — | `src/routes/admin/purge.ts:5` | Orphan (no frontend caller) |
