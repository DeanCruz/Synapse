...
Before touching any code, analyze a task to determine its full blast radius — which directories, files, and contracts would be affected. Helps the user understand complexity and make informed decisions about whether to proceed, simplify, or break the task into phases.
...
- **Files to modify:** {N}
- **Files to create:** {N}
- **Cross-layer contracts affected:** {N}
- **Estimated mode:** Serial / Parallel ({reason})

### Affected Files by Area

**{area_1} (e.g., API)** ({N} files)
- `path/to/file.ts` — {what changes and why}
- `path/to/file2.ts` — {what changes and why}

**{area_2} (e.g., Frontend)** ({N} files)
- `path/to/file.ts` — {what changes and why}

### Cross-Layer Impact

- {Shared type X must be updated in both frontend and backend}
- {API endpoint Y will change — frontend must update call}
- {Environment variable Z needs new value}
