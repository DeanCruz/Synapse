...
- Update the project knowledge graph at `{project_root}/.synapse/knowledge/` via `!learn_update` if the swarm created, moved, or restructured files
...
3. **Project knowledge graph for semantic discovery.** When filenames don't reveal purpose, or you need to understand how components relate, check `{project_root}/.synapse/knowledge/` if it exists. If no knowledge graph exists and the project is large (500+ files), run `!learn`.
...
|---|---|
| `knowledge/` | Project knowledge graph generated by `!learn` and refreshed by `!learn_update` |
...
│   └── knowledge/                  ← Project knowledge graph (generated by !learn)
...
| `!health` | Project health check — AGENTS.md quality, dependency health, PKI coverage and staleness. |
...
| `!context {query}` | Query the knowledge graph, then supplement with grep/glob when needed. |
