...
1. Read `TableOfContentsMaster.md` to identify relevant repos and directories
2. Read the `CLAUDE.md` of every repo that might be involved
3. Use Grep/Glob to find all files related to the task
4. Read the key files to understand current implementation
5. Identify cross-repo contracts, shared types, and config dependencies
...
Based on the plan:
- **Serial** if ≤3 changes in a single repo with dependencies between them
- **Parallel** if 3+ independent changes, especially across repos
...
#### 1. {Change title}
- **File:** `{repo/path/file.ext}` ({create / modify})
...
### Cross-Repo Impact
- {Any shared types, API contracts, or config that spans repos}
