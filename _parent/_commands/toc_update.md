...
**Purpose:** Incrementally update `TableOfContentsMaster.md` by detecting new, deleted, and moved files since the last TOC generation. Dispatches parallel agents only for directories with changes — much faster than a full `!toc_generate`. Use this for routine maintenance after adding features, creating files, or making small structural changes.

**Syntax:** `!toc_update`

**Produces:** An updated `{parent_directory}/TableOfContentsMaster.md` with new files added, deleted files removed, and stale entries corrected.
...
Read `{parent_directory}/TableOfContentsMaster.md` in full. Parse every file entry to build an **indexed set** of all currently documented file paths.

### Step 2: Scan the filesystem

For each child repo listed in the TOC (and any new directories in the parent that are NOT in the TOC), scan the file tree using Glob:

```
Glob: {child_repo}/src/**/*.ts
Glob: {child_repo}/src/**/*.tsx
Glob: {child_repo}/_commands/**/*.md
Glob: {child_repo}/**/*.md (for docs)
... (adapt patterns per repo's tech stack)
```

Also scan workspace-level directories:
```
Glob: _commands/**/*.md
```
...
- **New repos** — a child directory in the workspace with no TOC entry at all
...
### New Files
- `my-backend/src/services/NewService.ts`
- `my-frontend/src/hooks/explore/useExploreFilters.ts`
- ...

### Deleted Files
- `my-frontend/src/lib/services/oldService.ts`
- ...

### New Repos/Directories
- `new-repo/` (not in TOC at all)
- ...
```
...
- **A completely new repo** → one agent per top-level source directory (same as `!toc_generate` would do for that repo)
...
```
You are scanning new files that need to be added to the workspace Table of Contents.
...
4. **Related files** — Direct imports/dependencies/consumers (paths relative to workspace root)
5. **Exports** — Key exported symbols other files consume

## Context
{Relevant repo CLAUDE.md excerpt — architecture, conventions, tech stack}
...
Read `{parent_directory}/TableOfContentsMaster.md` again (in case of context compaction).
...
- If removing the last directory in a repo section, keep the repo heading but note it's empty

#### Additions
- Insert new file entries into the correct repo and directory section
...
- If a completely new repo was found, create its full section header with summary, CLAUDE.md status, and tags (read its CLAUDE.md if available)
- Follow the exact formatting of existing entries — bold filenames, inline summaries, bracketed tags, indented related/exports

#### Workspace Overview
- If new repos were added, update the Workspace Overview table and Relationships section at the top of the TOC
- If repos were removed, remove them from the overview
...
Write the updated TOC to `{parent_directory}/TableOfContentsMaster.md`.
...
- **The master agent does NOT read source files.** Agents read new files. The master only reads the TOC, directory listings, CLAUDE.md files, and agent returns.
