...
**Purpose:** Search the workspace Table of Contents to quickly locate files by topic, keyword, or object name. Returns the most relevant file(s) with their paths and context — faster than blind Glob/Grep searching.
...
- `{search_parameters}` — Descriptive search (e.g., `backend rate limiting`, `frontend styling colors`, `how generations work`)

**Examples:**
```
!toc SimulationService
!toc explore page hooks
!toc backend auth middleware
...
Read `{parent_directory}/TableOfContentsMaster.md` in full. This is the workspace's semantic index — it contains summaries, tags, related files, and context for every indexed file across all repos.
...
**1. {File name}**
- **Path:** `{full_path_relative_to_workspace}`
- **Repo:** {child_repo_name}
...
- Which repo or directory might contain what they're looking for
