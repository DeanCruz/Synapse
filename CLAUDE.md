...
The PKI is a persistent knowledge layer at `{project_root}/.synapse/knowledge/` that accumulates deep operational understanding of the target project -- gotchas, patterns, conventions, domain taxonomy, file relationships, and swarm-level insights. It is populated by five mechanisms: `!learn` (cold-start bootstrap), worker annotations (swarm-time discovery), a PostToolUse staleness hook (automatic change detection), `!learn_update` (incremental refresh), and **post-swarm knowledge extraction** (automatic after every swarm — harvests worker annotations and generates swarm-level insights). Masters use the PKI during pre-planning to inject relevant knowledge into worker prompts. Workers query the PKI before implementing to surface gotchas and patterns. The `PromptBuilder` auto-injects PKI knowledge into worker prompts during GUI-dispatched swarms. The `!context` command queries it for enriched output. The system gets smarter with every swarm run. See [`documentation/project-integration/pki-overview.md`](documentation/project-integration/pki-overview.md) for full details.
