...
**Key fields:** `task_id`, `status`, `started_at`, `completed_at`, `summary`, `assigned_agent`, `stage`, `message`, `milestones[]`, `deviations[]`, `logs[]`, `shared_context`, `sibling_reads[]`

The progress file now contains the **full lifecycle** for each agent. The `shared_context` object provides optional data for same-wave sibling coordination (sub-fields: `exports`, `interfaces`, `patterns`, `notes`). The `sibling_reads` array records which sibling progress files were read, enabling the dashboard to render sibling communication lines. The `logs[]` array feeds the popup log box in the agent details modal. The `status`, `started_at`, `completed_at`, `summary`, and `assigned_agent` fields live exclusively in progress files — the dashboard derives all stats from these files.
...
Swarm performance metrics, computed by the master agent after swarm completion. Contains elapsed time, worker efficiency ratios, task duration distribution, and parallelism statistics. Located at `{tracker_root}/dashboards/{dashboardId}/metrics.json`. Written once at the end of a swarm — not updated during execution.
