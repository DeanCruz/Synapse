...
0. **Read your task spec from `plan.json`.** Open `{tracker_root}/dashboards/{dashboardId}/plan.json`. Read `context` (the original user prompt + shared conventions/constraints that apply to ALL agents) and the `tasks[]` entry where `id` matches your `task_id`. That entry contains the master's deeply-thought `approach` and the exact `files` you should touch. **This is your canonical task spec** — your dispatch prompt summarizes it, but `plan.json` is the source of truth. If `plan.json` is missing or your task entry is absent, write a CRITICAL deviation and stop.
