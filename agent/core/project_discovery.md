...
1. **Glob/Grep first for targeted searches.** They cost zero context tokens and are always current. Use them alongside the `.synapse/knowledge/` graph.
...
3. **Project knowledge graph for semantic discovery.** When filenames don't reveal purpose, or you need to understand how components relate, check `{project_root}/.synapse/knowledge/` if it exists. If none exists and the project is large (500+ files), run `!learn`.
...
| `knowledge/manifest.json` | PKI routing index with file entries, hashes, staleness flags, and insights |
| `knowledge/annotations/{hash}.json` | Per-file operational knowledge, gotchas, patterns, conventions, and relationships |
| `knowledge/domain_index.json` | Domain-to-file routing index |
| `knowledge/tag_index.json` | Tag-to-file routing index |
| `knowledge/concept_map.json` | Concept-to-file routing index |
