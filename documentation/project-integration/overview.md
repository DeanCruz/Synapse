...
- `.synapse/` metadata directory (config and PKI knowledge graph)
...
2. `{project_root}/.synapse/knowledge/` (if it exists) for PKI data -- routing, gotchas, patterns, conventions, and relationships
...
└── .synapse/              # Add to .gitignore
    ...
    └── knowledge/         # Project Knowledge Index (PKI) — persistent knowledge graph
```
...
A semantic project knowledge graph with routing indexes and per-file operational annotations. Generated by `!learn` and incrementally updated by `!learn_update`. See [Project Knowledge Graph](./toc-system.md) for details.
...
Synapse supports working on multiple projects simultaneously through its multi-dashboard system. Each dynamic hex-ID dashboard can serve a different project.
...
|---|---|---|
| 71894a | `/Users/dean/repos/frontend` | Refactor components |
| da7091 | `/Users/dean/repos/frontend` | Add dark mode |
| 540931 | `/Users/dean/repos/api` | Database migration |

To switch between projects:
- Use the Code sidebar to switch dashboards
- Use the active dashboard's Project button to edit its project binding
- Use `!project set` or `--project` as CLI fallback paths
